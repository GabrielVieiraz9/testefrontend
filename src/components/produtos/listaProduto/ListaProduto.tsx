...
function ListaProdutos() {
  ...
  async function buscarProdutos() {
    try {
      await buscar(`/produtos/filtro?nome=${searchTerm}&categoria=${selectedCategory}`, setProdutos, {
        headers: {
          Authorization: token,
        },
      });
...
<div className='flex flex-wrap justify-center items-center w-full gap-28'>
    ...
    className="border-2 border-slate-700 rounded p-2 w-full md:w-1/2"></input>
    <select
    onChange={(e) => setSelectedCategory(e.target.value)}
    name="categoria" id="categoria" className='border p-2 border-slate-800 rounded w-full md:w-1/12'>
